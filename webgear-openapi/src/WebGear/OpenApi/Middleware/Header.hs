...
mkParam :: forall name val. (KnownSymbol name, ToSchema val)
        => Proxy name -> Proxy val -> Bool -> Param
mkParam _ _ isRequired = (mempty :: Param)
  ...
  setTrait WG.Header _ =
    let
      ...
      header = mempty @Header
               ...
               & schema ?~ Inline (toSchema $ Proxy @val)
    in
      OpenApiHandler $ singletonNode (DocResponseHeader headerName header)
