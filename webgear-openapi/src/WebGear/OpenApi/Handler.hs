...
data Tree a = NullNode
            | SingleNode a (Tree a)
            | BinaryNode (Tree a) (Tree a)
            deriving stock (Show)

data DocNode = DocSecurityScheme Text SecurityScheme
...
postOrder (BinaryNode t1 t2) doc f = f $
...
mergeDoc (DocStatus status) child doc =
  preOrder child doc $ \doc' ->
    let
      opr = mempty @Operation & at (HTTP.statusCode status) ?~ Inline (mempty @Response)
      pathItem = mempty @PathItem
                 & get ?~ opr
                 & put ?~ opr
                 & post ?~ opr
                 & delete ?~ opr
...
mergeDoc (DocResponseBody defs mediaType mediaTypeObject) child doc =
    postOrder child doc $ \doc' ->
      let
        resp = mempty @Response & content <>~ [(mediaType, mediaTypeObject)]
      in
        doc'
...
mergeDoc (DocResponseHeader headerName header) child doc =
  postOrder child doc $ \doc' ->
    ...
    in
      doc' & allOperations.responses.responses %~ Map.map (`swaggerMappend` Inline resp)
